...
app_dt <- app_dt[,!(names(app_dt) %in% col)]
...
skewness(app_dt$LTV_TRANSFORMED)
...
table(app_dt$DAYS_EMPLOYED_YEARS)
...
for (i in 1: length(nm)){
  nm1 <- nm[i]
  temp <- data.frame(c(nm1),sum(is.na(app_dt[,i])))
  colnames(temp) <- c("Variable","NA_cnt") 
  missing_dt_summary <- rbind(missing_dt_summary,temp)
}


write.csv(app_dt,paste0(path,"/04_Interim/application_train_processed_V1.csv"), row.names=F)
